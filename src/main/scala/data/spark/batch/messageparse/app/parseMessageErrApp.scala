    ...
    sparkContext.hadoopFile(args(0),classOf[TextInputFormat], classOf[LongWritable], classOf[Text], 1)
      ...
      if (line.length == 354) {
        Map(
          ...
          "amt_trans" -> line.substring(66, 78).trim.toDouble / 100,
          ...
          "recycle_fee" -> line.substring(166, 178).trim.toDouble / 100,
          "pay_fee" -> line.substring(179, 191).trim.toDouble / 100,
          "installment_pay_fee" -> pattern.findAllIn(line.substring(192, 204).trim).mkString.toDouble / 100,
          "amt_trans_fee" -> pattern.findAllIn(line.substring(205, 217).trim).mkString.toDouble / 100,
          "receive_fee" -> line.substring(218, 230).trim.toDouble / 100,
          "cost_fee" -> line.substring(231, 243).trim.toDouble / 100,
          ...
          "amt_trans2" -> line.substring(337, 349).trim.toDouble / 100,
